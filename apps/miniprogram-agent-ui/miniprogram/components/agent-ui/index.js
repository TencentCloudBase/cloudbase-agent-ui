  ...
  methods: {
    ...
    transformToolCallHistoryList: function (toolCallList) {
      ...
      for(let i = 0;i < callParamsList.length;i++) {
        ...
        const curResult = toolCallList.find(item => item.type === "tool-result" && item.toolCallId === curParam.tool_call.id)
        const curContent = callContentList[i]
        const curError = toolCallList.find(item => item.finish_reason === "error" && item.error.message.toolCallId === curParam.tool_call.id)
