...
Component({
  ...
  methods: {
    ...
    submitFeedback: async function () {
      ...
      const res = await cloudInstance.extend.AI.bot.sendFeedback({
        userFeedback: {
          ...
          tags:
            this.data.feedbackType === "upvote"
              ? this.data.upVote.filter((item) => item.selected).map((item) => item.value)
              : this.data.downVote.filter((item) => item.selected).map((item) => item.value),
